    ...
    public partial class BaseForm
    {
        private void navigator_SelectMeshEvent()
        {
            ...
            try
            {
                ...
                rows.Add(new RowProperty("", new ButtonPropertyValue("Показать",
() => {
    ShowElements(1,true);
    DisplayObjects();
})));
                rows.Add(new RowProperty("", new ButtonPropertyValue("Скрыть",
() => {
    ShowElements(1, false);
    DisplayObjects();
})));
                rows.Add(new RowProperty("", new ButtonPropertyValue("Удалить",
() => {
    DelElements(1);
    DisplayObjects();
})));

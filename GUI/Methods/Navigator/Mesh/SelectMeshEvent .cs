    ...
    public partial class BaseForm
    {
        private void navigator_SelectMeshEvent()
        {
            ...
            try
            {
                ...
                rows.Add(new RowProperty("Показать", new ButtonPropertyValue("Ok",
() => {
    ShowElements(1,true);
    DisplayObjects();
})));
                rows.Add(new RowProperty("Скрыть", new ButtonPropertyValue("Ok",
() => {
    ShowElements(1, false);
    DisplayObjects();
})));
                rows.Add(new RowProperty("Удалить", new ButtonPropertyValue("Ok",
() => {
    DelElements(1);
    DisplayObjects();
})));

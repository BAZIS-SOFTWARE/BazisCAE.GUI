        ...
        public override List<RowProperty> GetRowProperty()
        {
            ...
            else
            {
                ...
                property.Add(_media.HeatExchangeFunc == "*"
                    ...
                    : RowProperty.CreateComboBox("Коэф. теплоотдачи", _media.HeatExchangeFunc, _func, true, ValidationType.None));
                property.Add(_media.TemperatureFunc == "*"
                    ? RowProperty.CreateTextBox("Температура среды", _media.TemperatureValue.ToString())
                    : RowProperty.CreateComboBox("Температура среды", _media.TemperatureFunc, _func , true, ValidationType.None));
            }
